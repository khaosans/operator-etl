...
GitHub Actions runs `./harness/e2e.sh` on every push — includes PII leak, critic faithfulness, MCP allowlist, HITL routing, and path-traversal tests (76 pytest total). Security workflow: bandit + pip-audit. Human guide: [docs/SECURITY-HARDENING.md](docs/SECURITY-HARDENING.md).
