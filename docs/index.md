...
Success ends with **`OPERATOR_ETL_VERIFY=PASS`**, 34 pytest, and a FOIA demo of `status=complete`, `silver=10`, `quarantined=2`.
...
| 34 pytest + CI on every push | Live GCP / BigQuery end-to-end |
| MCP allowlist (3 tools, no vault) | LLM-generated insights (templates today) |
...
4. **Use** — [CLI](CLI.md) · [DASHBOARD](DASHBOARD.md) · [MCP](MCP.md) · [ADD-A-SOURCE](ADD-A-SOURCE.md)
